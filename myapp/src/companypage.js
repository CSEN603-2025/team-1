...
function CompanyPage() {
  ...
  const [allApplicantsFilter, setAllApplicantsFilter] = useState({
    // For the main applicants tab
    status: "",
    search: "",
    jobTitle: "",
  })
  ...
  useEffect(() => {
    if (storedCompany?.companyEmail) {
      ...
      const jobsWithEnsuredIds = companyJobs.map((job, index) => ({
        ...
        id:
          job.id ||
          `job_${storedCompany.companyEmail}_${Date.now()}_${index}_${Math.random().toString(36).substring(2, 9)}`,
      }))
  ...
  const handleJobModalToggle = (editMode = false) => {
    ...
    if (!editMode || !isJobModalOpen) {
      // Reset if opening for new or closing
  ...
  const handleUpdateApplicantStatus = (applicantEmail, newStatus, title) => {
    ...
    // 1. Update the job's applicants in postedJobs state
    ...
    // 2. Update global appliedInternships in localStorage - FIXED to match by both email AND title
    const allAppliedString = localStorage.getItem("appliedInternships") || "[]"
    let allApplied = []
    try {
      allApplied = JSON.parse(allAppliedString)
      if (!Array.isArray(allApplied)) allApplied = []
    } catch (e) {
      console.error("Error parsing appliedInternships", e)
      return
    }
    ...
    // 4. Update selected applicant if it's the same one
  ...
  console.log(updated)
      ...
      let currentInterns = []
      try {
        currentInterns = JSON.parse(localStorage.getItem(companyInternsKey) || "[]")
      } catch (e) {
  ...
  const theme = {
    primary: "#6b46c1",
    primaryTransparent: "rgba(47, 2, 68, 0.42)",
    primaryLight: "#e9d8fd",
    primaryDark: "#553c9a",
  ...
  const menuItems = [
    ...
    {
      text: "Post a Job",
      onClick: () => handleJobModalToggle(false),
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="18"
    ...
    },
    {
      ...
      state: { companyEmail: storedCompany?.companyEmail },
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="18"
    ...
    >
      ...
      <div
        style={{
          ...
          background: `linear-gradient(to bottom, ${theme.primaryTransparent}, rgba(107, 70, 193, 0.3))`,
        }}
      >
        {menuOpen && (
          <>
      ...
      >
        ...
        {isPopupOpen && (
          ...
          >
            ...
            >
              <h4 style={{ margin: 0, fontSize: "16px", fontWeight: "600", color: theme.neutral.darkest }}>
                Notifications
              </h4>
              <button
            ...
            {notifications.length === 0 ? (
              <div style={{ padding: "20px 0", textAlign: "center", color: theme.neutral.medium, fontSize: "14px" }}>
                No notifications
              </div>
            ) : (
              notifications.map((notification, index) => (
                ...
                >
                  <p style={{ margin: "0 0 5px 0", fontSize: "14px", fontWeight: "500", color: theme.neutral.darkest }}>
                    {notification.message}
                  </p>
                  <p style={{ margin: 0, fontSize: "12px", color: theme.neutral.medium }}>
        ...
        <div style={{ padding: "30px" }}>
          ...
          >
            ...
            <p style={{ fontSize: "15px", color: theme.neutral.medium, margin: 0 }}>
          ...
          {activeTab === "jobs" && (
            <div>
              ...
              >
                <h2 style={{ margin: 0, fontSize: "18px", fontWeight: "600", color: theme.neutral.darkest }}>
                  {companyName}'s Posted Jobs
                </h2>
                <button
                  onClick={() => handleJobModalToggle(false)}
                  style={{ ...buttonStyle.primary, backgroundColor: theme.primary }}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
              ...
              >
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", alignItems: "center" }}>
                  ...
                  >
                    <span
                      style={{ marginRight: "12px", fontWeight: "500", fontSize: "14px", color: theme.neutral.medium }}
                    >
                      Payment:
                    </span>
                    <label
                    ...
                    {["1 month", "2 months", "3 months"].map((duration) => (
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => setFilterDuration((prev) => ({ ...prev, [duration]: e.target.checked }))}
                          style={{ width: "14px", height: "14px", accentColor: theme.primary }}
                        />
                        {duration}
                      </label>
              ...
              {filteredJobs.length > 0 ? (
                  ...
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {filteredJobs.map(
                          (
                            job, // job object here has a unique 'id'
                          ) => (
                            <tr
                              key={job.id}
                              style={{
                              ...
                              <td style={{ padding: "14px 20px" }}>
                                <span
                                  style={{
                                    ...
                                    padding: "4px 10px",
                                    backgroundColor:
                                      job.applicants?.length > 0 ? theme.primaryLight : theme.neutral.lightest,
                                  ...
                                  onClick={() =>
                                    ...
                                    handleViewApplicants(
                                      job,
                                      postedJobs.findIndex((pJob) => pJob.id === job.id),
                                    )
                                  }
                                >
                ...
                >
                  ...
                  <h3
                    style={{ margin: "0 0 8px 0", fontSize: "18px", fontWeight: "600", color: theme.neutral.darkest }}
                  >
                  ...
                  <button onClick={() => handleJobModalToggle(false)} style={buttonStyle.primary}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="16"
          ...
          {activeTab === "applicants" && (
            <div>
              <h2 style={{ margin: "0 0 24px 0", fontSize: "18px", fontWeight: "600", color: theme.neutral.darkest }}>
                All Applicants
              </h2>
              ...
              >
                <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", marginBottom: "10px" }}>
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <input
                        ...
                        value={allApplicantsFilter.search}
                        onChange={(e) => setAllApplicantsFilter({ ...allApplicantsFilter, search: e.target.value })}
                        style={{
                          padding: "10px 12px 10px 36px",
                          width: "calc(100% - 24px)",
                          border: `1px solid ${theme.neutral.light}`,
                  ...
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    <select
                      value={allApplicantsFilter.jobTitle}
                      onChange={(e) => setAllApplicantsFilter({ ...allApplicantsFilter, jobTitle: e.target.value })}
                      style={{
                        padding: "10px 12px",
                        width: "100%",
                        border: `1px solid ${theme.neutral.light}`,
                    ...
                    >
                      ...
                      {Array.from(new Set(postedJobs.map((job) => job.title))).map((title, index) => (
                  ...
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    <select
                      value={allApplicantsFilter.status}
                      onChange={(e) => setAllApplicantsFilter({ ...allApplicantsFilter, status: e.target.value })}
                      style={{
                        padding: "10px 12px",
                        width: "100%",
                        border: `1px solid ${theme.neutral.light}`,
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {filteredAllApplicants.map((applicant, index) => (
                          <tr
                            key={applicant.email + (applicant.jobId || index)}
                            style={{
                              borderBottom: `1px solid ${theme.neutral.light}`,
                              transition: "background-color 0.2s",
                            }}
                          >
                            ...
                            <td style={{ padding: "14px 20px", textAlign: "right" }}>
                              <button
                                onClick={() => {
                                  const jobIndex = postedJobs.findIndex((job) => job.id === applicant.jobId)
                                  if (jobIndex !== -1) {
                                    setCurrentJobIndex(jobIndex)
                                    setSelectedJobApplicants(postedJobs[jobIndex].applicants || [])
                                    handleViewApplicantProfile(applicant)
                                  } else {
                                    console.warn("Original job not found for applicant")
                                    handleViewApplicantProfile(
                                      applicant,
                                    ) /* Still show profile, but job context might be limited */
                                  }
                                }}
              ...
              <form onSubmit={handleJobSubmit}>
                ...
                >
                  <input
                    ...
                    onChange={handleJobInputChange}
                    style={{ width: "16px", height: "16px", marginRight: "12px", accentColor: theme.primary }}
                  />
                  <label
                    htmlFor="isPaid"
                    style={{ fontSize: "14px", fontWeight: "500", color: theme.neutral.darkest, cursor: "pointer" }}
                  >
                    Paid Position
                  </label>
                </div>
                ...
                <div style={{ marginBottom: "16px" }}>
                  ...
                  />
                  <p style={{ margin: "4px 0 0 0", fontSize: "12px", color: theme.neutral.medium }}>
              ...
              {filteredModalApplicants.length > 0 ? (
                ...
                >
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      {filteredModalApplicants.map((applicant, index) => (
                        <tr
                        ...
                        >
                          ...
                          <td style={{ padding: "14px 20px", textAlign: "right" }}>
                            <button onClick={() => handleViewApplicantProfile(applicant)} style={buttonStyle.primary}>
                              <svg
                                width="14"
                                height="14"
                                viewBox="0 0 24 24"
                ...
                >
                  <p style={{ margin: 0, fontSize: "14px", color: theme.neutral.medium }}>
                    No applicants match your filters for this job.
                  </p>
                </div>
              )}
              <button onClick={handleCloseApplicantsModal} style={buttonStyle.secondary}>
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    <div>
                      ...
                      <p style={{ margin: 0, fontSize: "14px", color: theme.neutral.darkest }}>
                ...
                >
                  ...
                  {selectedApplicant.skills ? (
                    ...
                    <p style={{ margin: 0, fontSize: "14px", color: theme.neutral.medium, fontStyle: "italic" }}>
                      No skills listed
                    </p>
                  )}
                </div>
              ...
              >
                ...
                {selectedApplicant.experience ? (
                  <p style={{ margin: 0, fontSize: "14px", color: theme.neutral.darkest, lineHeight: "1.5" }}>
                    {selectedApplicant.experience}
                  </p>
                ) : (
                  <p style={{ margin: 0, fontSize: "14px", color: theme.neutral.medium, fontStyle: "italic" }}>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", marginBottom: "16px" }}>
                  <p style={{ margin: "0 12px 0 0", fontSize: "14px", color: theme.neutral.darkest }}>
                    Current Status:
                  </p>
                  <div
                    style={{
                ...
                <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                  <button
                    onClick={() => {
                      handleUpdateApplicantStatus(selectedApplicant.email, "finalized", selectedApplicant.jobTitle)
                    }}
                  ...
                  <button
                    onClick={() => {
                      handleUpdateApplicantStatus(selectedApplicant.email, "accepted", selectedApplicant.jobTitle)
                    }}
                    style={{
                  ...
                  <button
                    onClick={() => {
                      handleUpdateApplicantStatus(selectedApplicant.email, "rejected", selectedApplicant.jobTitle)
                    }}
              ...
              <button onClick={handleCloseProfileModal} style={buttonStyle.secondary}>
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
